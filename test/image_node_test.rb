...
class ImageNodeTest < Test::Unit::TestCase
   ...
   def test_basics
      ...
      assert_nil   image.x_scaling
      assert_nil   image.y_scaling
      assert_nil   image.top_crop
      assert_nil   image.right_crop
      assert_nil   image.left_crop
      assert_nil   image.bottom_crop
   end
   ...
   def test_mutators
      ...
      image.x_scaling = 75
      assert_equal 75, image.x_scaling

      image.y_scaling = 60
      assert_equal 60, image.y_scaling

      image.top_crop = 100
      assert_equal 100, image.top_crop

      image.bottom_crop = 10
      assert_equal 10, image.bottom_crop

      image.right_crop = 35
      assert_equal 35, image.right_crop

      image.left_crop = 50
      assert_equal 50, image.left_crop

      image.displayed_width = 5000
      assert_equal 5000, image.displayed_width

      image.displayed_height = 9000
      assert_equal 9000, image.displayed_height
   end
   ...
   def test_image_reading
      ...
      assert_equal 20, images[0].width
      assert_equal 20, images[0].height
      assert_equal 123, images[1].width
      assert_equal 456, images[1].height
      assert_equal 20, images[2].width
      assert_equal 20, images[2].height
      assert_equal 123, images[3].width
      assert_equal 456, images[3].height
      assert_equal 20, images[4].width
      assert_equal 20, images[4].height
      assert_equal 123, images[5].width
      assert_equal 456, images[5].height
   end
   ...
   def test_rtf
      ...
      assert_equal "{\\*\\shppict{\\pict\\picscalex100\\picscaley75"\
